...
use crate::errors::PNGMEErrors::{InvalidHeader, UnknownChunkType};
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let removed_chunk_position = self
            .chunks
            .iter()
            .position(|c| c.chunk_type().to_string() == chunk_type)
            .ok_or(UnknownChunkType())?;

        let removed_chunk = self.chunks.remove(removed_chunk_position);
        Ok(removed_chunk)
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        let (header, no_header_value) = value.split_at(Png::STANDARD_HEADER.len());
        ...
        if !(header
            ...
            .all(|(a, b)| a == b))
        {
            return Err(Box::new(InvalidHeader(header.try_into()?)));
        }

        let mut index = 0;
        let mut chunks = Vec::new();

        while index < no_header_value.len() {
            let bytes = &no_header_value[index..];
            let chunk = Chunk::try_from(bytes)?;

            index += (chunk.length() as usize) + Chunk::TOTAL_BYTES;
            chunks.push(chunk);
        }
